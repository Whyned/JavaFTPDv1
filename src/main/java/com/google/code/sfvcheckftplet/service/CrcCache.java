...
public class CrcCache {
	...
	public void putFileCrc(File file, Long checksum) {
		...
		logger.debug("Adding crc info ("+checksum+") for file " + absolutePath);
		if(absolutePath.contains("(incomplete)-")){
			logger.debug("Looks like someone is filling into incomplete folder. Fixxing absolute path.");
			absolutePath = absolutePath.replace("(incomplete)-", "");
		}
		cache.put(new Element(FILE_PREFIX + absolutePath, checksum));
		cache.flush();
	}
	...
	public void removeFileCrc(File file) {
		String absolutePath = file.getAbsolutePath();
		logger.debug("Removing crc info for file " + absolutePath);
		if(absolutePath.contains("(incomplete)-")){
			logger.debug("Looks like someone is filling into incomplete folder. Fixxing absolute path.");
			absolutePath = absolutePath.replace("(incomplete)-", "");
		}
